...
class Utils():
    ...
    @staticmethod
    def cleanString(s, b):
        table = {ord(char): None for char in b}
        temp_str = s.translate(table)
...
#            self.output("[DEBUG]   " + inspect.getframeinfo(prev_frame).filename + ":" + str(inspect.getframeinfo(prev_frame).lineno), end=end, flush=True, rewrite=rewrite)
    ...
    def printModuleList(self, modules):
        ...
        self.output("+-" + "".ljust(module_len, "-") + "-+-" + "".ljust(type_len, "-") + "-+-" + "".ljust(desc_len, "-") + "-+")
        self.output("| " + "Module".ljust(module_len) + " | " + "Type".ljust(type_len) + " | " + "Description".ljust(desc_len) + " |")
        self.output("+-" + "".ljust(module_len, "-") + "-+-" + "".ljust(type_len, "-") + "-+-" + "".ljust(desc_len, "-") + "-+")

        sort_modules = sorted(modules, key=lambda x: (modules[x]['type'], modules[x]['name']))
        for module in sort_modules:
            self.output("| " + modules[module]['name'].ljust(module_len) + " | " + modules[module]['type'].ljust(type_len) + " | " + modules[module]['description'].ljust(desc_len) + " |")
